...
from bot.utils.helpers import get_current_month, get_current_date, get_user_now
from bot.utils.temp_storage import get_temp_data, store_temp_data
...
class CallbackHandler(BaseHandler):
    async def handle(self, event):
                ...
                if cb_data['type'] == 'debt_wallet':
                    ...
                    if debt:
                        ...
                        OutstandingHistoryRepository.add(
                            user_id, name, -amount,
                            get_current_date(event), get_current_time_str(event)
                        )
        ...
        # Goal handlers
            ...
            await event.edit(t(user_id, 'goal_manager'), buttons=btns)
    ...
    # Helper methods for outstanding/debt/goal functions
    async def show_out_names(self, event, dtype, page):
        ...
        total_pages = math.ceil(len(all_names) / 10)
        start = page * 10
    ...
    async def show_out_repay_list(self, event, dtype, page):
        ...
        slice_rows = rows[page*10:(page+1)*10]
        btns = [[Button.inline(f"{r['name']} ({r['amount']})", f"orp_{dtype}_{r['id']}")] for r in slice_rows]
        nav = []
        if page > 0:
    ...
    async def show_out_grid(self, event):
        user_id = event.sender_id
        receivable = OutstandingRepository.get_by_type(user_id, "give")
        payable = OutstandingRepository.get_by_type(user_id, "take")
        buttons = []
        for i in range(max(len(receivable), len(payable))):
            row = []
            if i < len(receivable):
                d = receivable[i]
                row.append(Button.inline(f"⬆️ {d['name']} {d['amount']}", f"ovdet_{d['name']}"))
            else:
    ...
    async def show_out_person_history(self, event, name, page):
        ...
        total_pages = math.ceil(total_logs / 10)
        
        msg = f"{t(user_id, 'out_history', name)}\n━━━━━━━━━━━━━━━━━━\n"
        if not slice_logs:
            msg += t(user_id, 'no_transactions')
        for l in slice_logs:
            msg += f"🔹 {l['amount']} | {l['date']} | {l['time']}\n"
    ...
    async def show_repay_list(self, event, dtype, page):
        ...
        total_pages = math.ceil(len(rows) / 10)
        slice_rows = rows[page*10:(page+1)*10]
        btns = [[Button.inline(f"{r['name']} ({r['amount']})", f"rp_{dtype}_{r['id']}")] for r in slice_rows]
        nav = []
        if page > 0:
    ...
    async def show_debt_grid(self, event):
        ...
        for i in range(max(len(receivable), len(payable))):
            ...
            if i < len(receivable):
                d = receivable[i]
                row.append(Button.inline(f"⬆️ {d['name']} {d['amount']}", f"vdet_{d['name']}"))
            else:
                row.append(Button.inline("-", b"noop"))
            if i < len(payable):
                d = payable[i]
    ...
    async def show_person_history(self, event, name, page):
        ...
        msg += f"{t(user_id, 'page')}: {page + 1} / {max(1, total_pages)}\n\n"
        if not logs:
            msg += t(user_id, 'no_transactions')
        for l in logs:
            msg += f"🔹 {l['amount']} | {l['date']} | {l['time']}\n"
    ...
    async def show_goal_list(self, event, page):
        ...
        total_pages = math.ceil(len(rows) / 10)
        slice_rows = rows[page*10:(page+1)*10]
        btns = [[Button.inline(f"{r['name']} ({r['saved']}/{r['target']})", f"gsave_{r['name']}")] for r in slice_rows]
        nav = []
        if page > 0:
